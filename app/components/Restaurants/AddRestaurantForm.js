  ...
  const imageSelect = async () => {
    ...
    if (resultPermissions === "denied") {
      toastRef.current.show(
        "Es necesario aceptar los permisos de galeria, si los has rechazado tienes que ir ha ajustes y activarlos manualmente.",
        3500
      );
    } else {
...
export default function AddRestaurantForm(props) {
  ...
  const addRestaurant = () => {
    if (!restaurantName || !restaurantAddress || !restaurantDescription) {
      ...
      toastRef.current.show("Selecciona la ubicación del restaurante", 2000);
    } else {
